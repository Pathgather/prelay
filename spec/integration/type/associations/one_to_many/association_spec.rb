...
class OneToManyAssociationSpec < PrelaySpec
  ...
  it "should support fetching associated items through a one-to-many association" do
    ...
    assert_sqls [
      ...
      %(SELECT "tracks"."id", "tracks"."name", "tracks"."release_id", "tracks"."created_at" FROM "tracks" WHERE ("tracks"."release_id" IN ('#{album.id}')) ORDER BY "created_at" LIMIT 50)
    ]
  end
  ...
  it "should support fetching cursors but no nodes" do
    ...
    assert_sqls [
      %(SELECT "albums"."id", "albums"."name" FROM "albums" WHERE ("albums"."id" = '#{album.id}')),
      %(SELECT "tracks"."release_id", "tracks"."created_at" FROM "tracks" WHERE ("tracks"."release_id" IN ('#{album.id}')) ORDER BY "created_at" LIMIT 50)
    ]
  end
  ...
  it "should support fetching pagination info but no edges" do
    ...
    assert_sqls [
      %(SELECT "albums"."id", "albums"."name" FROM "albums" WHERE ("albums"."id" = '#{album.id}')),
      %(SELECT "tracks"."id", "tracks"."release_id" FROM "tracks" WHERE ("tracks"."release_id" IN ('#{album.id}')) ORDER BY "created_at" LIMIT 51)
    ]
  end
end
