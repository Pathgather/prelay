    ...
    class << self
      ...
      def build_graphql_object(config)
        ...
        config.resolve -> (inputs, ctx) {
          ...
          result = {}
          selections = GraphQLProcessor.process(ctx, schema: schema).selections
              ...
              result[result_field.name] =
                if (id = ids.fetch(normalized){raise "Mutation #{to_s} returned a results hash without a '#{normalized}' key!"})
                  if result_field.association
                    ...
                    resolver = RelayProcessor.new(selection, target_types: [result_field.target_type], entry_point: entry_point).to_resolver(order: Sequel.desc(:created_at)){|ds| ds.where(id: id)}
                    record = resolver.resolve_singular
